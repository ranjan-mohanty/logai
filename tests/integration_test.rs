...
fn test_json_log_parsing_and_grouping() {
    let content = fs::read_to_string("tests/fixtures/sample.log").expect("Failed to read sample.log");
    ...
    let error_count = entries.iter().filter(|e| matches!(e.severity, Severity::Error)).count();
    ...
    assert!(groups.len() <= error_count, "Groups should be <= total errors");
    
    // Verify group has correct structure
    let first_group = &groups[0];
    assert!(!first_group.pattern.is_empty(), "Pattern should not be empty");
...
fn test_cloudwatch_log_parsing() {
    ...
    let has_errors = entries.iter().any(|e| matches!(e.severity, Severity::Error));
...
fn test_spring_boot_log_grouping() {
    ...
    let connection_errors = groups.iter()
        .find(|g| g.pattern.contains("Connection timeout"));
    
    assert!(connection_errors.is_some(), "Should find connection timeout errors");
    
    if let Some(group) = connection_errors {
        assert_eq!(group.count, 2, "Should group 2 connection timeout errors together");
    }
}
...
fn test_nginx_log_grouping() {
    ...
    let connection_refused = groups.iter()
        .find(|g| g.pattern.contains("Connection refused"));
    
    assert!(connection_refused.is_some(), "Should find connection refused errors");
    
    if let Some(group) = connection_refused {
        assert_eq!(group.count, 2, "Should group 2 connection refused errors together");
    }
}
...
fn test_empty_log_file() {
    ...
    let analyzer = Analyzer::new();
    let groups = analyzer.analyze(entries).expect("Should handle empty input");
...
fn test_no_errors_in_logs() {
    ...
    let entry = parser.parse_line(content).expect("Should parse").expect("Should have entry");
    
    let analyzer = Analyzer::new();
    let groups = analyzer.analyze(vec![entry]).expect("Analysis should succeed");
    
    assert!(groups.is_empty(), "Info logs should not create error groups");
}
...
fn test_dynamic_value_normalization() {
    ...
    assert_eq!(groups.len(), 1, "Should group all 'User not found' errors together");
